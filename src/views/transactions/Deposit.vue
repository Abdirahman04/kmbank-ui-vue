<template>
    <div class="deposit container">
        ...
        <div class="row">
            <div class="row">
                <div class="col lavender"><h4>How much do you want to deposit?</h4></div>
            </div>
        </div>
        <div class="row">
            <div class="row">
                <form class="p-3 border border-3 border-primary">
                    ...
                    <button id="btn" class="btn btn-info" @click.prevent="depositAmount(form, fetchBody)">Deposit</button>
                </form>
            </div>
...
<script>
import { deposit, login, getUserByAccNumberRaw, getUserByAccNumber } from '@/utils/apiService.js'
...
export default {
    ...
    methods: {
        ...
        depositAmount(loginBody, depositBody) {
            login(loginBody).then(data => {
                if(data) {
                    deposit(depositBody).then(res => {
                        if (!res.ok) {
